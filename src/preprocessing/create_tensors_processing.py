...
CHECKPOINT_PATH = Path(__file__).resolve().parents[2] / "config/checkpoints/checkpoint_tensor.txt"
...
for f in folders:
	...
	for p in ["_protein_", "_pocket_"]:
		...
		fil=len(numpy.nonzero(N_tensor)[0])
		#print(fil, "filled voxels")
		emp=len(numpy.nonzero(N_tensor==0)[0])
		...
		numpy.savetxt(path_folder+"for_labview"+p+"/xyz.txt", xyz, fmt="%1.8f")
		...
		numpy.save(path_folder+"for_labview"+p+"/N_tensor.npy", N_tensor)
		...
		nn+=1
		print(round(nn/len(folders)*100/2,4), " percent completed")
		print()
